DROP DATABASE IF EXISTS les_4;
CREATE DATABASE les_4;
...
JOIN likes l ON l.user_id = m.user_id
WHERE (YEAR(CURRENT_DATE)-YEAR(`birthday`)) <= 12
GROUP BY u.id;
...
-- Проверяем все ли лайки мы извлекли
SELECT COUNT(*) FROM likes;

-- Убеждаемся, что в исходной табюлице тоже 20 лайков
...
-- Кол-во писем по участникам переписки
SELECT 
	from_user_id AS 'id отправителя',
	to_user_id AS 'id получателя',
...
SELECT 
	fr.initiator_user_id AS 'Отправитель сообщения',
    fr.target_user_id AS 'Получатель сообщения',
    status AS 'Статус',
    COUNT(m.id) AS 'Кол-во сообщений'
FROM friend_requests fr
JOIN messages m ON fr.initiator_user_id = m.from_user_id OR fr.target_user_id = m.to_user_id
WHERE status = 'approved'
